...
1. **Export**:
    - **1 Default Path**: Resets the export path to the default (installation location, Exports folder).
    - **2 Set Path**: Allows setting an export location for all generated files; a folder structure is created at the chosen path if it does not exist.
  
2. **Local Device Trees**:
    - The program allows storing and selecting multiple local device databases.
    - **3 Pull Folder from GitHub**: Load the local database stored on GitHub.
    - **4 Push Folder to GitHub**: Save the currently used local device tree folder to GitHub (Note: Unsaved changes will be pushed if the folder is not selected and saved before the process).
    - **5 Import from Local Folder**: Load the device trees from a folder on your computer.
    - **6 Save to Local Folder**: Save the currently loaded device trees to your computer (Note: Same conditions apply as for Push Folder).
    - **7 Select Device Trees**: Must always select local device trees for the generation process, even if they are empty!
    - **8 Discard Changes**: Discards all changes made.
    - **9 Apply Changes**: Applies and saves all changes made.
